...
$('#employee-rows').on('blur change', '.user-input', function() {
  ...
  if (isValid) {
    validBtn.removeClass('btn-secondary')
                .addClass('btn-success')
                .prop("disabled", false);
  } else {
    validBtn.removeClass('btn-success')
...
var timeRegex = /^([1-9]|[1][0-2]):?([0-5]\d)?\s?(?:AM|PM)$/i;
...
function formatTime(time) {
    ...
    if (newTime[2] == 'PM' && newTime[0] != 12) {newTime[0] += 12}
    else if (newTime[0] == 12 && newTime[2] == "AM") {newTime[0] = 0}
...
$('#employee-rows').on('change', '.timecard-input', function() {
    ...
    var hoursWorked = hours.concat(".", minutes)

    //how about find nearest of class
    var parent = $(this).parent().parent();
...
function validationErrors(err, context) {
    ...
    userErrors[myError['type']][myError['name']] = myError['text']
    // var errZeroValue = ""
...
$( "#validate-timecards5" ).on( "click", function() {
        ...
        $( ".row-employee" ).each(function() {
            if ($(this).find(".complete").is(":disabled")) {
                if (!($('.blur-select').val().length() == 0)) {
                    errors["Incomplete Row(s)"] = true;
                    //repeat anyway but add the row number
                }
            }
    ...
    function revenueMethod() {
        ...
        var lastRev = 0;
        $( ".rev-input" ).each(function() {
            if (lastRev > $(this).val()) {return false;}
            lastRev = $(this).val()
...
function securityRate() {
    // var timecards = {}
    requires standard hours worked

    create security tip rates by hours 
    pull all security
    find earliest start time // assume until close?
    complete the rates by hour... before 0, after 100, minutes as percentage

    
    var security = {
        '14': 0, '15': 0, '16': 0, '17': 0, '18': 0, '19': 0, '20': 0, '21': 0, '22': 0, '23': 0, '00': 0, '01': 0, '02': 0, '03': 0  
    }
    $( ".row-employee" ).each(function() {
...
$( "#validate-timecards" ).on( "click", function() {
    ...
    var tipsSum = tipsValues.reduce((a,v) => {
        return a+v;
      }, 0);
    if (tipsSum !== 100) {
    ...
    $( ".row-employee" ).each(function() {
        ...
        var duplicates = new Set(activeEmployees).size !== activeEmployees.length;
        if (duplicates) {errors.push("DupeEmp")}
        ...
        timecards[empID] = {
            ...
            'role': role,
            'earnings': {
                'wage': 16.28, //adjust by role, make editable by admin
            ...
            'hours': {
                'start': start,
                'end': end,
                'minsWorked': $(this).find('.hours-worked').data('mins-worked'), //do differently
    ...
    var formData = {
        ...
        'hours': hours,
        'rev-method': $("#rev-select").find('option:selected').val(), //need selected?
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            //error if peeps not working total hours
            ...
            for (const [key, value] of Object.entries(employeeData)) {
                ...
                //error for working same hour to same hour
            }
...
$('.user-edit').on('click', function(e) {
    ...
    $.ajax({
        ...
        url: "/users/1/edit",
        // data: {formData: JSON.stringify(formData)},
        data: {pk: JSON.stringify(pk)},
        headers: {'X-CSRFToken': csrftoken},
        success: function (response) {
            var user = response[pk]
            //fill values by unpacking dict?
